...
function Number({ mv, number, digitHeight }: { mv: MotionValue<number>; number: number; digitHeight: number }) {
    ...
    return (
        <motion.span style={{ y }} className='absolute inset-0 flex direction-reverse items-center justify-center'>
            {number}
        </motion.span>
    );
}
...
export function SlidingNumber({
    ...
    return (
        <div ref={ref} className={`flex items-center ${className} direction-reverse flex-row-reverse`}>
            {roundedValue < 0 && '-'}
            {places.map((place, idx) => {
                // For digit grouping (e.g., 2,000,000)
                // Left to right, so digit index 0 is the highest place (left).
                // Add comma before the digit if it's after every group of 3 (except for the first group).
                const digitIdx = places.length - idx; // From right: 1 (units), 2 (tens), etc.
                const isComma =
                    idx !== 0 && // not at the highest digit (leftmost)
                    (places.length - idx) % 3 === 0;

                return (
                    <span key={`${place}-${animationKey}`} className='relative flex items-center'>
                        {/* Digit */}
                        <Digit place={place} value={absValue} digitHeight={digitHeight} duration={duration} />
                        {/* Comma */}
                        {isComma && (
                            <span className='mx-0.5 select-none opacity-80 text-xl' aria-hidden='true'>
                                ,
                            </span>
                        )}
                    </span>
