...
#define DITHER_STRENGTH 1.0
    ...
    float g_inv(float x) {
        // Inverse gamma for sRGB (for blending on light backgrounds)
...
function getThemeFromDom(): 'light' | 'dark' {
    ...
    // fallback to light mode as default
    return 'light';
}
...
export const LaserFlow: React.FC<Props> = ({
    ...
    // Save and update theme value live (in case color-scheme changes)
    const themeRef = useRef<'light' | 'dark'>(
        typeof theme !== 'undefined' ? theme : typeof window === 'undefined' ? 'light' : getThemeFromDom()
    );

    // Listen to prefers-color-scheme changes (if no explicit prop)
    useEffect(() => {
        ...
        if (typeof window !== 'undefined' && window.matchMedia) {
            // React to changes in color-scheme
            const darkMq = window.matchMedia('(prefers-color-scheme: dark)');
            const lightMq = window.matchMedia('(prefers-color-scheme: light)');
            const handle = () => {
                if (darkMq.matches) themeRef.current = 'dark';
                else if (lightMq.matches) themeRef.current = 'light';
                else themeRef.current = 'light'; // fallback
                // Update uniform right away if present
                if (uniformsRef.current) {
                    uniformsRef.current.uLightMode.value = themeRef.current === 'light' ? 1 : 0;
                }
            };
            darkMq.addEventListener('change', handle);
            lightMq.addEventListener('change', handle);
            handle();
            return () => {
                darkMq.removeEventListener('change', handle);
                lightMq.removeEventListener('change', handle);
            };
        }
    }, [theme]);
    ...
    useEffect(() => {
        ...
        // Light mode uses white, dark mode uses black background
        const curTheme: 'light' | 'dark' = themeRef.current;
        renderer.setClearColor(curTheme === 'light' ? 0xffffff : 0x000000, 1);
        ...
        const animate = () => {
            ...
            // update uLightMode every frame in case of color-scheme change
            uniforms.uLightMode.value = themeRef.current === 'light' ? 1 : 0;
